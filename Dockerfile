...
CMD ["sh", "-c", "set -e; export SESSION_DRIVER=${SESSION_DRIVER:-file} CACHE_STORE=${CACHE_STORE:-file} QUEUE_CONNECTION=${QUEUE_CONNECTION:-sync} LOG_CHANNEL=${LOG_CHANNEL:-stderr}; if [ -z \"${APP_KEY}\" ]; then export APP_KEY=\"base64:$(php -r 'echo base64_encode(random_bytes(32));')\"; echo '[startup] APP_KEY ausente: chave temporaria gerada para este deploy'; fi; php artisan config:clear || true; php artisan cache:clear || true; php artisan migrate --force || true; php artisan db:seed --force || true; php artisan storage:link || true; php artisan serve --host=0.0.0.0 --port=${PORT:-8080}"]
