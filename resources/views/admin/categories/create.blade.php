...
<div class="mb-8">
    ...
    <p class="text-zinc-400">Adicione uma nova categoria de produtos</p>
</div>
    ...
    <form action="{{ route('admin.categorias.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description" 
                      name="description" 
                      rows="4"
                      class="w-full px-4 py-3 bg-zinc-950 border border-zinc-800/50 rounded focus:outline-none focus:border-zinc-700 transition">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
            @enderror
        </div>
